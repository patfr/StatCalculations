import { getConfigPage } from "../types/config.js";
import { Page, buildPage, createElement, createElementForEach } from "./page-builder.js";
function calculateTrans(amount, formula) {
...
const findTransFormula = (amount, formula) => getTransFormula(amount, formula)?.formula;
const getTransFormula = (amount, formula) => formula.find(f => f.amount <= amount) ?? formula[formula.length - 1];
const hasValue = (amount, formula) => formula.find(v => v.amount === amount) != undefined;
export class TranscensionPage extends Page {
    constructor(game) {
